...
The following is an awk trick that removes multiple spaces by default,
since the default output field separator (OFS) is a single space. In
awk, assigning to a field (\$1, \$2, etc.) causes the entire line (`$0`)
to be rebuilt from the fields, using the current OFS; the same effect
can be achieved with `$2=$2`.
